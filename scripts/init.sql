...
CREATE TABLE IF NOT EXISTS devices (
    ...
    device_type_id INTEGER REFERENCES device_types(id) ON DELETE
    SET NULL,
        location_id INTEGER REFERENCES locations(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS metrics (
    ...
    metric_type_id INTEGER REFERENCES metric_types(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS alerts (
    id SERIAL PRIMARY KEY,
    device_id INTEGER REFERENCES devices(id) ON DELETE CASCADE,
    severity_id INTEGER REFERENCES alert_severities(id) ON DELETE
    SET NULL,
        title VARCHAR(200) NOT NULL,
        message TEXT NOT NULL,
        is_resolved BOOLEAN DEFAULT FALSE,
        resolved_at TIMESTAMP,
        resolved_by INTEGER REFERENCES users(id) ON DELETE
    SET NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
VALUES (
    ...
    (
        'technician',
        'Técnico de TI con acceso a monitoreo y dispositivos'
    ),
    ('viewer', 'Solo puede visualizar el dashboard') ON CONFLICT (name) DO NOTHING;
INSERT INTO alert_severities (name, level, color, description)
VALUES (
        'info',
        1,
        '#3B82F6',
        'Información general, sin acción requerida'
    ),
    (
        'warning',
        2,
    ...
    (
        ...
        4,
        '#7C3AED',
        'Emergencia, sistema en riesgo'
    ) ON CONFLICT (name) DO NOTHING;
INSERT INTO device_types (name, description)
VALUES ('server', 'Servidor físico o virtual'),
    ...
    ('camera', 'Cámara de seguridad IP') ON CONFLICT (name) DO NOTHING;
INSERT INTO metric_types (name, unit, description)
VALUES ('cpu_usage', '%', 'Uso del procesador'),
    ...
    ('uptime', 'h', 'Tiempo en línea') ON CONFLICT (name) DO NOTHING;
